...
# -*- coding: utf-8 -*-
"""
PDF Text Extractor - Fast OCR Version
快速OCR版本，先处理少量页面验证功能
"""
...
class FastPDFTextExtractor:
    """快速PDF文本提取器 - 优先PyMuPDF，OCR处理少量页面"""

    def __init__(self, output_dir: str = "data/extracted/text_fast", max_ocr_pages: int = 10):
        ...
        try:
            ...
            for page_idx in range(total_pages):
                ...
                try:
                    ...
                    # 优先尝试PyMuPDF
                    page_blocks = self._try_pymupdf_extraction(page, page_num)

                    # 如果PyMuPDF失败且未达到OCR限制，尝试OCR
                    if not page_blocks and self.ocr_engine and ocr_page_count < self.max_ocr_pages:
                        logger.info(f"第{page_num}页：尝试OCR提取")
                        page_blocks = self._try_ocr_extraction(page, page_num)
                        if page_blocks:
                            ocr_page_count += 1
                            logger.info(f"OCR已处理 {ocr_page_count}/{self.max_ocr_pages} 页")
            ...
            logger.info(f"文本提取完成，成功处理 {pages_with_text}/{total_pages} 页，OCR处理了 {ocr_page_count} 页")
            return result
    ...
    def _save_text_content(self, pdf_path: Path, content: str) -> str:
        """保存文本内容"""
        output_filename = f"{pdf_path.stem}_fast_extracted.txt"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="快速PDF文本提取工具")
    parser.add_argument("input", help="输入PDF文件路径")
    parser.add_argument("-o", "--output", default="data/extracted/text_fast",
                       help="输出目录 (默认: data/extracted/text_fast)")
    parser.add_argument("-m", "--max-ocr-pages", type=int, default=10,
                       help="最大OCR处理页数 (默认: 10)")

    args = parser.parse_args()

    extractor = FastPDFTextExtractor(output_dir=args.output, max_ocr_pages=args.max_ocr_pages)
    ...
    try:
        ...
        print(f"OCR处理页数: {result['ocr_pages_processed']}")
